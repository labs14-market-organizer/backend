...
async function findReserveByDate(marketID, date) {
    ...
    const result = await db('market_booths as mb')
        .select('mb.id', 'm.admin_id as user_mkt', 'v.admin_id as user_vdr', 'mb.number', db.raw('(mb.number - count(mr.id)) as available'), db.raw('array_remove(array_agg(mr.vendor_id ORDER BY mr.vendor_id), NULL) as user_vdrs'))
        ...
        .join('markets as m', {'mb.market_id': 'm.id'})
